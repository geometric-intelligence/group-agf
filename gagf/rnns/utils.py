...
def plot_train_val_loss(
    train_loss_history, val_loss_history, save_path=None, show=True, xlabel="Step"
):
    """
...
def plot_wout_neuron_specialization(
    ...
    for step in steps:
        ...
        for j in range(H):
            ...
            F = np.fft.fft2(m)
            P = (F.conj() * F).real
            tp = [_tracked_power_from_fft2(P, kx, ky, p1, p2) for (kx, ky) in tracked_freqs]
        ...
        for j in range(R_ner * C_ner):
            ...
            if j < H:
                m = W[j].reshape(p1, p2)
                ax.imshow(m, vmin=vmin, vmax=vmax, origin="lower", aspect="equal", cmap=heat_cmap)
                # Colored border
                ec = edge_colors[j]
    ...
    handles = [
        Patch(facecolor="white", edgecolor=colors[i], linewidth=2.5, label=f"k={tracked_freqs[i]}")
...
def plot_wout_neuron_specialization_1d(
    ...
    for step in steps:
        ...
        fig, axes = plt.subplots(nrows, ncols, figsize=(2.5 * ncols, 1.5 * nrows), squeeze=False)
    ...
    handles.append(
        Patch(facecolor="white", edgecolor=(0.6, 0.6, 0.6, 1.0), linewidth=2.5, label="dead")
    )
